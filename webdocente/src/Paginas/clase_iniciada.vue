      ...
      <div id="app" class="fondo container-fluid" >
        ...
        <div class="row container d-flex justify-content-center align-items-center p-0 m-0">
          <div class="card-celeste">
            <h3 class="w-100 ramos text-center"> Clase iniciada <br> {{ inicio }}</h3>
          </div>
        </div>
        <!-- Botón de asistencia -->
        <div>
          <button type="button" class="btn boton_gris text-white bold" :class="{ 'boton-amarillo': botonC }" @click="claseiniciada">Finalizar Clase</button>
        </div>
      </div>
    </center>
  </template>
