<template>
  <!-- cargando -->
  <div
    class="container-fluid h-450 d-flex justify-content-center align-items-center"
    v-if="loading"
  >
  ...
  <!-- Tabla de horario -->
  <div
    class="row container-fluid w-100 mx-0 mt-3"
    v-if="!loading"
    :key="componentKey"
  >
    <!-- Header -->
    <div
      class="row rt-50 h-55 mx-0 font-20 bold primary-bg d-flex align-items-center"
    >
      <div class="col-1 text-center">Hora</div>
    ...
    <!-- Body -->
    <div
      class="row hover h-100 mx-0 font-16 bold secondary-bg mb-1 d-flex align-items-center"
    ...
    >
      ...
      <div
        class="col container hover-scale text-center"
        v-for="(dia, i) in dias"
        :key="i"
      ...
      >
        <p class="row">
          {{
            (getAsignacion(dia, index + 1).Ramo || "")
              .split(" ")
              .slice(0, 6)
              .join(" ")
          }}
...
export default {
  ...
  async mounted() {
    try {
      await axios
        .get(ENDPOINTS["bff-horarios"] + "/asignaciones/" + this.rut, {
          headers: {
    ...
    getAsignacion(dia, bloque) {
      try {
        let asignacion = this.asignaciones.find((asignacion) => {
          return (
            asignacion.Dia_Semana === dia.toString() &&
